...
import DoctorSupportChat from './doctor/DoctorSupportChat';
import { Users, Search, Globe, CalendarDays, MessageCircle } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { collection, serverTimestamp, query, where, onSnapshot, updateDoc, doc, getDocs, writeBatch, getDoc } from 'firebase/firestore';
...
export default function DoctorLayout() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, async (snap) => {
      if (!snap.empty) {
        ...
        const isUnread = snap.docs[0].data().unreadForDoctor;
        
        // Mark as delivered for unseen messages
        if (isUnread) {
          try {
            const msgsQuery = query(
              collection(db, `${COLLECTIONS.SUPPORT_CONVERSATIONS}/${convId}/messages`),
        ...
        if (isUnread && !previousUnreadRef.current && locationPathRef.current !== '/doctor/support') {
          // Toast for new message
          const toast = document.createElement('div');
          toast.className = 'fixed bottom-4 right-4 rtl:left-4 rtl:right-auto bg-blue-600 text-white px-6 py-4 rounded-xl shadow-lg z-50 flex flex-col gap-1 cursor-pointer transition-all duration-300 transform translate-y-0 opacity-100 hover:bg-blue-700';
          toast.innerHTML = `<span class="font-bold text-sm">${t('new_admin_message') || 'New message'}</span><span class="text-xs text-blue-100">${t('new_message') || 'Check your messages'}</span>`;
          toast.onclick = () => {
        ...
        previousUnreadRef.current = isUnread;
        setHasUnreadSupport(isUnread);
      }
  ...
  if (user?.role === 'doctor') {
    ...
    navItems.push({ name: t('doctor_support'), path: '/doctor/support', icon: MessageCircle, hasBadge: hasUnreadSupport });
  }
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 p-4 md:p-8 flex flex-col gap-6 overflow-y-auto">
          <Routes>
            ...
            <Route path="support" element={<DoctorSupportChat />} />
            {user?.role === 'doctor' && (
              <>
