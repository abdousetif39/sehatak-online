...
const [doctors, setDoctors] = useState<Doctor[]>([]);
  ...
  useEffect(() => {
    ...
    if (selectedSpecialty) {
      const spec = MEDICAL_SPECIALTIES.find(s => s.id === selectedSpecialty);
      if (spec) {
        result = result.filter(d => {
          if (d.specialtyId === selectedSpecialty) return true;
          const matchAr = d.specialtyAr === spec.ar || d.specialty === spec.ar;
          const matchFr = d.specialtyFr === spec.fr || d.specialty === spec.fr;
          return matchAr || matchFr;
        });
      }
    
    }
    setFilteredDoctors(result);
  }, [searchName, selectedState, selectedCity, selectedSpecialty, doctors]);
