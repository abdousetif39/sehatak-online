            ...
            <div className="shrink-0">
              {photoPreview ? (
                <div className="relative">
                  ...
                  <button type="button" onClick={() => { setPhotoPreview(''); setPhotoUrlInput(''); setFormData(prev => ({...prev, photoUrl: ''})) }} className="absolute -top-2 -right-2 p-1.5 bg-white text-rose-500 rounded-full shadow-sm hover:bg-rose-50 border border-slate-100 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
            ...
            <div className="flex-1 flex flex-col gap-3">
              <div>
                ...
                <div className="flex gap-2">
                  <input type="text" value={photoUrlInput} onChange={handlePhotoUrlChange} placeholder="https://..." className="flex-1 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:bg-white transition-colors" dir="ltr" />
                  <button type="button" onClick={handlePreviewImage} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-medium transition-colors whitespace-nowrap">
                    {t('preview_image')}
                  </button>
                </div>
              </div>
...
title={t("latin_letters_only")}
 value={formData.lastNameFr || formData.name?.split(' ').slice(1).join(' ') || ''} onChange={handleChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:bg-white transition-colors" dir="ltr" />
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('specialty_ar')}</label>
              <input required type="text" name="specialtyAr" value={formData.specialtyAr || formData.specialty || ''} onChange={handleChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:bg-white transition-colors" dir="rtl" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('specialty_fr')}</label>
              <input required type="text" name="specialtyFr" value={formData.specialtyFr || formData.specialty || ''} onChange={handleChange} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-600 focus:bg-white transition-colors" dir="ltr" />
            </div>
          </div>
        </div>
